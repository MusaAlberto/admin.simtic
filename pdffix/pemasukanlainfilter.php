<?php
	...
	class PDF extends FPDF{
		...
		function Header(){
			...
			$this->Cell(15,5,'ID',1,0,'',true);
			$this->Cell(30,5,'TANGGAL',1,0,'',true);
			$this->Cell(105,5,'SUMBER PEMASUKAN',1,0,'',true);
	...
	$query = mysqli_query($conn,"SELECT * FROM `apl_pemasukanlain` WHERE YEAR(Tanggal)='$tahun' AND MONTH(Tanggal)='$angka'");
	...
	$query2 = "SELECT sum(Jumlah) as jumlah FROM `apl_pemasukanlain` WHERE YEAR(Tanggal)='$tahun' AND MONTH(Tanggal)='$angka' ";
	...
	$pdf->Output("pemasukanlainfilter.pdf","D");
