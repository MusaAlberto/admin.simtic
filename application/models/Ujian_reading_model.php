<?php
class ujian_reading_model extends CI_Model {
	...
	public function cari_semua() {
		$QuerySaya 		= $this->db->query(
						  "SELECT tabel_soal_reading.*, tabel_kategori.kode_kategori as kd_kategori , tabel_kategori.nama_kategori as nama_kategori FROM tabel_soal_reading JOIN tabel_kategori ON tabel_soal_reading.tipe_soal = tabel_kategori.kode_kategori WHERE status<>'D' AND kategori='ujian';"
						);
	...
	public function tambah( $id, $kode, $gambar, $pertanyaan, $pil_a, $pil_b, $pil_c, $pil_d, $jawaban, $tipe) {
		$QuerySaya 		= $this->db->query(
						  ...
						  VALUES 
						  ('$id', '$kode', '$gambar', '$pertanyaan', '$pil_a', '$pil_b', '$pil_c', '$pil_d', '$jawaban', '$tipe', 'ujian', 'A');"
						);
		if ($QuerySaya) {
			return TRUE;
		} else {
