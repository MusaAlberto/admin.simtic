...
class Ujian_listening extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('ujian_listening_model');
	}
	...
	public function index() {
		...
		if ($level == 'dosen') {
			$data ['riwayat'] = $this->ujian_listening_model->riwayat_audio();
			$data ['tipe'] = $this->ujian_listening_model->getKategori();
			$this->load->view('soal_ujian/ujian_listening', $data);
		}
	}

	public function data() {
		$data					= $this->ujian_listening_model->cari_semua();		
		$hasil 					= array();
		$result 				= array();
		$nomor 					= 0;
	...
	public function tambah()
	{
		...
		if ($_POST['pil_a'] <> '') {
			...
			//rename nama file
			$config2['file_name']             = $this->ujian_listening_model->buat_kd_audio();
			...
			if ( ! $this->upload->do_upload('audio') && $riwayat_audio == false){
				...
				//rename nama file
				$config['file_name']             = $this->ujian_listening_model->buat_id();
			...
			$data 		= array(
				...
				'id_soal' => $this->ujian_listening_model->buat_id(),
				'id_audio'=> $this->ujian_listening_model->buat_kd_audio(),
				'gambar'=>$file_name,
			...
			$query=$this->ujian_listening_model->tambah('tabel_soal_listening', $data);
			$query=$this->ujian_listening_model->tambah('tabel_audio', $data_audio);
		} else{
			$status = false;
		}
	...
	public function edit() {
		...
		if ($_POST['pil_a'] <> '') {
			...
			//query memanggil fungsi tambah di model
			$query=$this->ujian_listening_model->edit('tabel_soal_listening', $data, $this->input->post('id_soal', TRUE));
			$query=$this->ujian_listening_model->edit_audio('tabel_audio', $data_audio, $this->input->post('id_audio', TRUE));
		} else{
			$status = false;
		}
			//mengubah respon menjadi json agar bisa dibaca ajax
	...
	public function hapus($id) {
		$hapus 				= $this->ujian_listening_model->hapus($id);
		$msg 				= false;
		if ($hapus) {
			$msg 			= true;
		}
