...
class Simulasi_reading extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('simulasi_reading_model');
	}
	...
	public function index() {
		...
		if ($level == 'dosen') {
			$data ['tipe'] = $this->simulasi_reading_model->getKategori();
			$this->load->view('soal_simulasi/simulasi_reading', $data);
		}
	}

	public function data() {
		$data					= $this->simulasi_reading_model->cari_semua();		
		$hasil 					= array();
		$result 				= array();
		$nomor 					= 0;
	...
	public function tambah() {
		if (isset($_POST['pil_a'])) {
			...
			$id_soal			= $this->simulasi_reading_model->buat_id();
			...
			//config untuk image
				...
				$id					= $id_soal;
				$kode				= $this->simulasi_reading_model->buat_kd();		
				$simpan 			= $this->simulasi_reading_model->tambah(
										$id,
										$kode,
										$file_name,
										$_POST['pertanyaan'],
	...
	public function edit() {
		if (isset($_POST['pil_a'])) {
			...
			if (isset($_FILES['foto']['name'])) {
				...
				$config['max_size']             = 5000;
				//rename nama file
				$config['file_name']            = $this->simulasi_reading_model->buat_id();
				$this->load->library('upload', $config);
				if ($this->upload->do_upload('foto')){
			...
			$edit 				= $this->simulasi_reading_model->edit(
									$_POST['id'],
									$image,
									$_POST['pertanyaan'],
									$_POST['pil_a'],
	...
	public function hapus($id) {
		$hapus 				= $this->simulasi_reading_model->hapus($id);
		$msg 				= false;
		if ($hapus) {
			$msg 			= true;
		}
